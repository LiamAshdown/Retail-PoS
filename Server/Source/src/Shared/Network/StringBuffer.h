...
namespace SteerStone
{
    ...
    public:
        ...
        explicit StringBuffer(std::size_t p_ReserveSize = STORAGE_INITIAL_SIZE) : m_WritePosition(0)
        {
    ...
    public:
        ...
        void AppendString(std::string const p_Buffer, bool p_Delimeter = true)
        {
            if (std::size_t l_Length = p_Buffer.length())
                Append((uint8 const*)EncodeStringBase64(p_Buffer).c_str(), l_Length);
        ...
        void AppendInterger(uint32 const p_Value)
        {
            std::string l_Encoder = EncodeIntergerBase64(p_Value, sizeof(uint32));
            if (std::size_t l_Length = l_Encoder.length())
                Append((uint8 const*)EncodeIntergerBase64(p_Value, sizeof(uint32)).c_str(), l_Length);
        }
        ...
        void AppendSOH()
        {
            ...
            if (std::size_t l_Length = l_SOH.length())
                Append(EncodeStringBase64(l_SOH).c_str(), l_Length);
        }
        ...
        void AppendSOT()
        {
            ...
            if (std::size_t l_Length = l_SOT.length())
                Append(EncodeStringBase64(l_SOT).c_str(), l_Length);
        }
        ...
        /// Clear - clear our storage and m_WritePosition
        void Clear()
        {
            m_WritePosition = 0;
            m_Storage.clear();
