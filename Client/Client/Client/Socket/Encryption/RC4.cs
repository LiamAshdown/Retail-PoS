...
namespace SteerStone.Encryption
{
    ...
    public class RC4
    {
        ...
        {
            byte[] m_Temp = new byte[p_Data.Length];

            System.Text.ASCIIEncoding l_Encoding = new System.Text.ASCIIEncoding();
            m_Temp = l_Encoding.GetBytes(p_Data);

            m_Data = Array.ConvertAll(m_Temp, q => Convert.ToSByte(q));
        }
